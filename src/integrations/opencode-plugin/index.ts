...
/**
 * Resolve the worker port from the user's persisted claude-mem settings so
 * OpenCode talks to the same worker as hooks, the viewer, and the NPX CLI.
 */
function resolveWorkerPort(): string {
  return SettingsDefaultsManager.loadUserSettings().CLAUDE_MEM_WORKER_PORT;
}
